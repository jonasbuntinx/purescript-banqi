...
pieceScore board@(Board squares) piece =
  let
    remaining =
      fromFoldable $ map (\l -> Tuple (NES.head l) (NES.length l))
        $ group
        $ sort
        $ squares
        # mapMaybe case _ of
  ...
  in
    case piece of
      ...
      { color, label: Soldier } ->
        if count { color: flipColor color, label: General } /= 0 then
          5 - count { color, label: Soldier }
        else
          1
      { color, label: Cannon } -> 1

captureScore :: Board -> Position -> Score
captureScore board pos =
  sum $ apply [ up, down, left, right ] [ pos ]
...
evaluate player board@(Board squares) =
  sum
    ...
    >>> \pos -> case _ of
        ...
        FaceUp piece@{ color, label }
          | label == Cannon -> weight color (sum [ pieceScore board piece, cannonScore board pos ])
...
abMinimax depth player alpha beta board
  ...
  | otherwise = do
    ...
    if player == state.turn then
      pure <<< maximum' =<< maxAbMap alpha beta (abMinimax (depth - 1) (flipColor player)) newBoards
    else
      pure <<< minimum' =<< minAbMap alpha beta (abMinimax (depth - 1) (flipColor player)) newBoards
    where
    newBoards =
      possibleActions board player
        # filter case _ of
            Turn _ -> false
            _ -> true
